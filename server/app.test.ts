...
describe("design scaffold locks", () => {
  it("boots helix-theme onto html.light and keeps dark as :root", () => {
    ...
    expect(css).toMatch(/html\.no-backdrop \{[\s\S]*?--bg-elevated:\s*#1c1c1e/);
    expect(css).toMatch(/html\.light\.no-backdrop \{[\s\S]*?--bg-elevated:\s*#ffffff/);
    expect(css).toMatch(/html\.no-backdrop \.chrome \{[\s\S]*?background:\s*var\(--solid\)/);
    expect(css).toMatch(/html\.no-backdrop \.chrome \{[\s\S]*?backdrop-filter:\s*none/);
    expect(readFileSync("src/lib/chrome.ts", "utf8")).toMatch(/no-backdrop/);
    expect(readFileSync("src/lib/chrome.ts", "utf8")).toMatch(/backdropFilterComputes/);
    expect(readFileSync("src/main.tsx", "utf8")).toMatch(/syncBackdropChrome/);
    ...
    expect(sheet).toMatch(/padding: 10px 20px calc\(28px \+ var\(--dock-h\)\)/);
    expect(css).toMatch(/\.sheet \.btn,[\s\S]*?background: var\(--primary\)/);
    expect(css).toMatch(/\.sheet \.btn,[\s\S]*?backdrop-filter: none/);
    expect(css).toMatch(/\.theme-overlay \.btn,[\s\S]*?background: var\(--primary\)/);
    expect(css).toMatch(/\.theme-overlay \.btn,[\s\S]*?backdrop-filter: none/);
    expect(css).toMatch(/\.account-logout \{[\s\S]*?safe-area-inset-bottom/);
    expect(css).toMatch(/\.cal-nav button \{[\s\S]*?width: 44px[\s\S]*?height: 44px[\s\S]*?min-width: 44px/);
    expect(css).toMatch(/\.cal button \{[\s\S]*?min-width: 44px[\s\S]*?min-height: 44px/);
    expect(css).toMatch(/\.subnav a \{[\s\S]*?min-height: 44px/);
    expect(css).toMatch(/\.subnav a \{[\s\S]*?min-width: 44px/);
  ...
  it("maps locked Today / You / Vitals copy onto this tree", () => {
    ...
    expect(readFileSync("src/pages/Vitals.tsx", "utf8")).not.toMatch(/Loading…/);
    expect(readFileSync("src/pages/Protocol.tsx", "utf8")).not.toMatch(/Loading…/);
    expect(readFileSync("src/pages/Calendar.tsx", "utf8")).not.toMatch(/Loading…/);
    expect(readFileSync("src/pages/Account.tsx", "utf8")).not.toMatch(/Loading…/);
    expect(readFileSync("src/components/Sheets.tsx", "utf8")).not.toMatch(/Loading…/);
    expect(readFileSync("src/App.tsx", "utf8")).not.toMatch(/Loading…/);
  ...
  it("puts a peptide calculator under Protocol", () => {
    ...
    expect(readFileSync("src/styles.css", "utf8")).not.toMatch(/\.calc-row select \{[^}]*5\.5rem/);
    expect(readFileSync("src/styles.css", "utf8")).not.toMatch(/\.calc-row select \{[^}]*min-width:\s*88px/);
...
describe("auth page lock", () => {
  ...
  it("keeps the You Face ID overlay as a client lock", () => {
    ...
    expect(you).toContain("face-id-msg");
    expect(you).toContain("account-logout");
    expect(you).not.toContain("Face ID was cancelled.");
    expect(you).not.toContain("Face ID is not available on this device.");
